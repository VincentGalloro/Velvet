...
class VerticalSplitFixedLeftLayout(private val topHeight: Double,
    ...
    override fun top(bounds: Bounds) = bounds.setHeight(min(topHeight, bounds.size.height), 0.0)
    override fun bottom(bounds: Bounds) = bounds.setHeight(max(bounds.size.height-topHeight-sepHeight, 0.0), 1.0)
}

class VerticalSplitFixedRightLayout(private val bottomHeight: Double,
                                    private val sepHeight: Double) : VerticalSplitLayout{
    override fun top(bounds: Bounds) = bounds.setHeight(max(bounds.size.height-bottomHeight-sepHeight, 0.0), 0.0)
    override fun bottom(bounds: Bounds) = bounds.setHeight(min(bottomHeight, bounds.size.height), 1.0)
}
